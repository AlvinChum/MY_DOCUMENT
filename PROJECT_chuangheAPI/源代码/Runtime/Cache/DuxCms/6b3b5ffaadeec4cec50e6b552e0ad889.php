<?php if (!defined('THINK_PATH')) exit();?><h3><?php echo ($config["gzhid"]); ?>列表</h3>
...
<input  type="hidden" name="dux_table_name" value="<?php echo ($dux_table_name); ?>"> 
<div class="formitm form-submit">
        <div class="ipt">
            ...
            <a href="<?php echo U('create_menu',array('do'=>$do));?>" class="u-btn u-btn-primary" >先点击左边保存，然后再点击此处生成自定义菜单</a>
        </div>
        </div>  
   	...
   	$("tbody tr").each(function(){
		...
		if( $(this).find("input").eq(1).attr('id')!='first_value[]'){
			if((!thisid1 && thisival2 ) || (thisid1 && !thisival2 ) ){
				$(this).addClass("red1");
				alert("蓝色区域有误。二级菜单两个必填！或者都不填。景诺软件手写代码！");
				sss =0;
			}
		}
