 ...
 * 后台菜单
 */
class AdReleaseModel extends Model {
    ...
    public function countList($where = array()){
    	return $this->table("__AD_RELEASE__ as A")
     ...
     * @return int 数量
     */
    public function dj_countList($where = array()){
    	return $this->table("__AD_LOG__ as A")
    	->join('__USERS__ as B ON A.uid = B.id')
    	->where($where)
    	->count();
    }
    ...
    public function loadList($where = array(), $limit = 0){
    	$data   = $this->table("__AD_RELEASE__ as A")
    	->join('__USERS__ as B ON A.uid = B.id')
     ...
     * @return array 列表
     */
    public function dj_loadList($where = array(), $limit = 0){
    	$data   = $this->table("__AD_LOG__ as A")
    	->join('__USERS__ as B ON A.uid = B.id')
    	->field('A.*,B.user_login')
    	->where($where)
    	->limit($limit)
