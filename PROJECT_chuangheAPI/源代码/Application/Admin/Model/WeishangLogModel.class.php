 ...
 * 后台菜单
 */
class WeishangLogModel extends Model {
    ...
    public function countList($where = array()){
    	return $this->table("__WEISHANG_LOG__ as A")
    	->join('__USERS__ as B ON A.uid = B.id')
    	->where($where)
    	->count();
    }
    
    public function cz_countList($where = array()){
    	return $this->table("__CHONGZHI_LOG__ as A")
    	->join('__USERS__ as B ON A.uid = B.id')
    	->where($where)
    	->count();
    }
    ...
    public function loadList($where = array(), $limit = 0){
    	$data   = $this->table("__WEISHANG_LOG__ as A")
    	->join('__USERS__ as B ON A.uid = B.id')
    ...
    public function cz_loadList($where = array(), $limit = 0){
    	$data   = $this->table("__CHONGZHI_LOG__ as A")
    	->join('__USERS__ as B ON A.uid = B.id')
    	->field('A.*,B.user_login')
    	->where($where)
    	->limit($limit)
