    ...
    public function add()
    {
        ...
        if (!IS_POST)
        {
            ...
            if ($model->saveData('add',$_POST['fieldset_id'])){
            	if($this->formInfo['table']=='monegzh'){
            		$id = $model->getLastInsID();
            		if($id){            			
            			$url = 'http://'.$_POST['Fieldset_domain'].U('Home/Public/wxlogin',array('gowxurl'=>$id));
            			$model->where('data_id = '.$id)->setField('cdtzurl',$url);
            		}
            ...
            if ($model->saveData('edit',$_POST['fieldset_id']))
            {
            	...
            	if($_POST['Fieldset_status']==1 && $_POST['Fieldset_money'] >0 && $_POST['Fieldset_uid']>0 && $this->formInfo['table']=='tixian'){            		
            		...
            		if($money>=$_POST['Fieldset_money']){
            			$desc='支付宝提现';
						A("Home/Site")->changemoney((-1)*$_POST['Fieldset_money'],7,$desc,$_POST['Fieldset_uid'],$note=1,$ip=0);
				...
				if($this->formInfo['table']=='monegzh'){
					...
					if($id){
						$url = 'http://'.$_POST['Fieldset_domain'].U('Home/Public/wxlogin',array('gowxurl'=>$id));
						$model->where('data_id = '.$id)->setField('cdtzurl',$url);
					    if(S('mo_wx_config'.$id)) S('mo_wx_config'.$id,NULL);
					}
				
				}
				//二维码海报
				if($this->formInfo['table']=='qrcode'){
					if($_POST['Fieldset_is_use']){
						M('Ext_qrcode')->where(array('data_id' => array('neq', $_POST['data_id'])))->save(array('is_use'=>0));
					}
				}
