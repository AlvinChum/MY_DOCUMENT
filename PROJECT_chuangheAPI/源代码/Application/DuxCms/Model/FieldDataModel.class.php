...
class FieldDataModel extends Model {
     ...
     * @return bool 更新状态
     */
    public function saveData($type = 'add' , $fieldsetInfo,$data_id=null){//2015-3-17 新增  $data_id=null
        //获取字段列表
        
       if(!is_array($fieldsetInfo)){
        	$fieldList=D('DuxCms/Field')->loadList('fieldset_id='.$fieldsetInfo);
        }else
            ...
            case '6':
                ...
                if(!empty($list)){
                    foreach ($list as $key => $value) {
                        $html.="<a href=".$value['url']." target='_blank'>".$value['url'].'</a><br>';
                    }
                }
                return $html;
                break;
   ...
   public  function  unser($data){
       ...
       if(is_array($data_array)){
       	...
       	$data_str = count($data_str)>2?implode('，', $data_str):$data_str[0];
