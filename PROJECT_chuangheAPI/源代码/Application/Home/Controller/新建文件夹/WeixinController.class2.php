			...
			switch($Event){
				...
				if($re['type']=='newreg'){//新关注	 	
					...
					if($eventkey&& $xml['Ticket']){//二维码关注
						...
						A('Api')->awardToParent(1,'jifen',$re['id'],$pid);	 
						
					}
					
					A('Api')->awardToParent(1,'money',$re['id'],$pid);	 
...
public function repalceTA($str){//shanmao.me  20151022  采集相关   http://shanmao.me/webback/thinkphp/php-cai-ji-mo-shou-shi-jie-shu-ju
    ...
    for($i=0,$j=count($arr[0]); $i<$j; $i++){
      ...
      $str = str_replace($arr[0][$i],'src="http://img01.store.sogou.com/net/a/04/link?appid=100520031&w=710&url='.$arr[1][$i].'"',$str);
	  else{
...
$xjson = '{ 
			   ...
			   "sub_button":[
					{
					   "type":"view",
					   "name":"马上赚钱",
					   "url":"http://888.shanmao.me/index.php?s=/Home/Index/index2.html"
					},
					{
					   "type":"view",
					   "name":"进入首页",
					   "url":"http://888.shanmao.me"
					},
					{
					   "type":"view",
					   "name":"邀请得现金",
					   "url":"http://888.shanmao.me/index.php?s=/Home/User/yaoqing.html"
					},
					{
					   "type":"view",
					   "name":"积分摇红包",
					   "url":"http://888.shanmao.me/index.php?s=/Home/Huodong/yaoyiyao.html"
					}
	...
	*/
	...
	if($re['errcode']==40001) S("gettokenf",null);
