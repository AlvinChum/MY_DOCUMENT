...
class ContentArticleModel extends Model {
     ...
     * @return array 列表
     */
    public function loadList($where = array(), $limit = 0, $order = 'A.time desc,A.content_id desc',$uid=0){
        $where['C.app'] = 'Article';
        ...
        if(!empty($pageList)){
            ...
            foreach ($pageList as $key=>$value) {
                $list[$key]=$value;
                $list[$key]['aurl'] = D('Article/ContentArticle')->getUrl($value,$uid);
     ...
     * @return bool 删除状态
     */
    public function getUrl($info,$uid=0)
    {
        return U('Article/Content/index',array('content_id'=>$info['content_id'],"uid"=>$uid,"time"=>time(),"fxapi2"=>1));
    }
