...
(function($){
    $.fn.extend({
        'lunbo':function(){
            ...
            var l=0;
            var timer=null;
            var len=$li.length*2; //复制了一份图片，长度变了。
            ...
            var ul_width=Math.abs(-len*w/2)
            //自动每8秒切换一次
            /*
            timer=setInterval(init,8000);
            function init(){
               $next.trigger('click'); //当页面一加载就触发next按钮的点击事件，用trigger的好处是减少重复代码,如果不用，就要把按钮click事件里代码全部复制过来，因为加载的代码和点击next按钮代码和效果相同，所以就用trigger触发执行click里面代码。
            }
...
*/
            ...
            function showCurrent(l){     //把图片的左右切换封装成一个函数
                ...
                $ul.find('.active').removeClass('active').parent().find('li').eq(Math.abs(l)/w).addClass('active');
