...
<base target="_self" />
<script>
var ajaxurl='getdata.php?type=PC';
...
<div class="tug">
<!--配乐-->
    <audio id="audio_player1" src="./style/images/countdown.mp3"></audio>
    <audio id="audio_player2" src="./style/images/tug-bg.mp3" loop="loop"></audio>
    <audio id="audio_player3" src="./style/images/cheers.mp3"></audio>
    ...
    <!--广告位-->
    <div class="track-adw">
		<a href="javascript:;"><img id="ad1" src="<?=!empty($ads[0])?$ads[0]:'./style/images/zanwu.jpg'?>"></a>
		<a href="javascript:;"><img id="ad2" src="<?=!empty($ads[1])?$ads[1]:'./style/images/zanwu.jpg'?>"></a>
		<a href="javascript:;"><img id="ad3" src="<?=!empty($ads[2])?$ads[2]:'./style/images/zanwu.jpg'?>"></a>
		<a href="javascript:;"><img id="ad4" src="<?=!empty($ads[3])?$ads[3]:'./style/images/zanwu.jpg'?>"></a>
    </div>
    
    <!--倒计时 //时间改动 -->
    <div class="tug-time">
    	<span class="sec" id='sec'><?=$bhconf['timelimit']?></span>
    ...
    <div class="tug-team">
    	<div class="team-red">
        	<span class="team-box">
                ...
                <p><?=$bhconf['teama']?><br /><span id='team1_cnt'>0</span>人</p>
            </span>
        </div>
        <div class="team-blue">
        	<span class="team-box">
            	<img src="<?=$bhconf['teambpic']?>">
                <p><?=$bhconf['teamb']?><br /><span id='team2_cnt'>0</span>人</p>
            </span>
        </div>
    </div>
    ...
    <div class="play-box">
        ...
        <img src="./style/images/play-line.png" class="paly-line">
    </div>
    ...
    <div class="masktips tips-left">
    	<p>1.关注公众号<span>
    	    <?php if($weixin){echo "【微信：".$weixinconf['dianplu_wxh']."】";} if($weibo){echo "【微博：".$weiboconf['nickname']."】";}?></span></p>
		<p>2.发送"<span><?=$plugconf['keyword']?></span>"，获得图文回复后，点击进入图文活动页</p>
        <p>3.进入比赛页后，请在大屏幕倒计时结束后，摇动手机参与比赛</p>
    </div>
    <?php if($weixin){ ?>
    <div class="masktips tips-right ">
    	<span style="width:110px;height:110px;display:block;background:#fff;position:relative;margin: 0 auto 10px;border-radius: 15px;overflow: hidden;">
        	<img src="<?=$weixinconf['erweima']?>" width="110" height="110" style="position:absolute;top:0;left:0">
        </span>
        <p><span class="yellow">微信</span>参与</p>
    </div>
    <?php
    }
    if($weibo){
    ?>
    <div class="masktips tips-right " style="right:180px">
    	<span style="width:110px;height:110px;display:block;background:#fff;position:relative;margin: 0 auto 10px;border-radius: 15px;overflow: hidden;">
        	<img src="<?=$weiboconf['erweima']?>" width="110" height="110" style="position:absolute;top:0;left:0">
        </span>
        <p><span class="yellow">微博</span>参与</p>
    </div>
    <?php }?>
    ...
    <div class="pop-winner">
    	<span id='win_span' class="team-box">
        	<img id='logo2' src="./style/images/pingju.jpg">
    ...
    <div class="pop-prize">
    	...
    	<div class="prize-box prize-box-left">
        	<div class="team-top">
            	<div class="team-info">
                	<span class="team-box"><img src="<?=$bhconf['teamapic']?>"></span>
					<?=$bhconf['teama']?>
                </div>
        ...
        <div class="prize-box prize-box-right">
        	<div class="team-top">
            	<div class="team-info">
                	<span class="team-box"><img src="<?=$bhconf['teambpic']?>"></span>
					<?=$bhconf['teamb']?>
                </div>
...
var size = 0;
//时间改动 
var time = <?=$bhconf['timelimit']?>00;
function chushihua(){
$.get(ajaxurl+'&op=chushihua', function(response){
});
}
$(document).ready(function() {
	...
	/* 最后一个人物背景图片带绳子 */
	$('.play-box-left .player-icon:first').css('background-image','url(./style/images/player-go-last-red.gif)');
	$('.play-box-right .player-icon:first').css('background-image','url(./style/images/player-go-last-blue.gif)');
	/* 拔河移动 */
	Tug = function(size){		
	...
	$('#Start').click(function(){
	    if($('#team1_cnt').text()==0 || $('#team2_cnt').text()==0){
	        alert('请等候两队都有参与人员再开始游戏哦！');
	        return false;
	    }
		...
		$.post(ajaxurl+'&op=tug_get_top',{vuid:13334}, function(response){
			...
			for(var top_cnt=0;top_cnt<7;top_cnt++)
			{
				if(typeof(response.topinfo['team1'][top_cnt]) != "undefined")
				{
					...
					if(pic.length<=0){pic="./style/images/tx-nopic.png"};
...
{	
	...
	$.post(ajaxurl+'&op=over_tug',{vuid:345}, function(response){
		if(response.game_over)
		{
			...
			{
				$('#logo2').attr('src','<?=$bhconf['teamapic']?>');
				$('#win_team_name').html('');
			...
			{
				$('#logo2').attr('src','<?=$bhconf['teambpic']?>');
				$('#win_team_name').html('');
			...
			{
				$('#logo2').attr('src','./style/images/pingju.jpg');
				$('#win_team_name').hide();
			...
			if(typeof(response.top_info[0]) != "undefined"){
				...
				if(response.top_info[0]['pic'].length<=0){response.top_info[0]['pic']="./style/images/tx-nopic.png"}
			...
			$.each(response.prize_setting, function(key, rows) {
				...
				for(var i =start;i<=end;i++)
				{
					if(typeof(response.team2_info[i]) != "undefined"){
						if(response.team2_info[i]['pic'].length<=0){response.team2_info[i]['pic']="./style/images/tx-nopic.png"}
			...
			$.each(response.prize_setting, function(key, rows) {
				...
				for(var i =start;i<=end;i++)
				{
					if(typeof(response.team1_info[i]) != "undefined"){
						if(response.team1_info[i]['pic'].length<=0){response.team1_info[i]['pic']="./style/images/tx-nopic.png"}
			...
			if(response.result==1)
			{
				$('.play-box-left .player-icon').css('background-image','url(./style/images/player-win-red.gif)');
				$('.play-box-right .player-icon').css('background-image','url(./style/images/player-lose-blue.png)');
			}else if(response.result==2)
			{
				$('.play-box-left .player-icon').css('background-image','url(./style/images/player-lose-red.png)');
				$('.play-box-right .player-icon').css('background-image','url(./style/images/player-win-blue.gif)');
			}else{
				$('.play-box-left .player-icon').css('background-image','url(./style/images/player-win-red.gif)');
				$('.play-box-right .player-icon').css('background-image','url(./style/images/player-win-blue.gif)');
			};
...
<?php
	echo '<img class="bg" src="./style/images/kuxuan.png"/><img class="bg2" src="./style/images/kuxuan2.png"/>';
?>
</body>
</html>