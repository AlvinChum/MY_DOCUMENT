...
class C
{
    ...
    //find query函数
    private function fquery($sql,$link,$type='assoc'){
        $key=md5($sql);
        $this->memkey=$key;
        // echo $sql;
    ...
    //update query函数
    private function upquery($where,$data){
        if($value=$this->find($where)){
            $mem=$this->mem;
            $key=$this->memkey;
            if(is_array($data)){
    ...
    //addquery 的函数
    private function aquery($sql,$link){
        $mem=$this->mem;
        if($mem){
            $cont=array(
    ...
    //find query函数
    private function squery($sql,$link,$type='assoc'){
        $key=md5($sql);
        $this->memkey=$key;
        // echo $sql;
        if(!empty($this->mem)){
    ...
    public function select($where = '1', $field = "*", $fun = '',$type = 'assoc')
    {
        ...
        if (empty($fun)) {
            ...
            $rarr = $this->squery($sqlStr, $this->link,$type);
        } else {
            $sqlStr = "select $fun($field) as rt from $this->table where $where";
            $rt = $this->fquery($sqlStr, $this->link);
            if ($rt) {
                if($type == "row"){
    ...
    public function find($where = '1', $field = "*", $fun = '',$type = 'assoc')
    {
        ...
        if (empty($fun)) {
            $sqlStr = "select $field from $this->table where $where";
            $rarr = $this->fquery($sqlStr, $this->link,$type);
            if(!$rarr){
                return false;
            }
        } else {
     ...
     */
    public function update($where, $data,$db=0)
    {
        $mem=$this->mem;
        
        if($mem){
            $return=$this->upquery($where,$data);
            if(!$db){
                return $return;
            }
        }
        ...
        return $this->query($sqlStr,$this->link);
    }
    ...
    public function add($data)
    {
        ...
        if($this->aquery($sqlStr,$this->link))
        {
